...
{
    if(voltage_value  < 0.20  || voltage_value > 4.0 ) // We will ignore values outside the range of measurement, this will happen around 20 - 150cm
    return(0);
...
void loop() {
 ...
 if(distance_value > 0 && distance_value < 30)
{
  closeby();
  updateValue();
}

if(distance_value >= 30 && distance_value < 50)
{
  mediumfar();
  updateValue();
}

if(distance_value >= 50 && distance_value < 150)
{
  faraway();
  updateValue();
}
...
value = analogRead(IR_PIN);       // reading our analog voltage, careful we only have 10 bit resolution so each
  ...
  distance_value = convert_to_distance(voltage_value); // getting actual distance value(cm) (careful using this, accuracy may not be ideal)
    ...
    if(distance_value  < 0  || distance_value > 200 ) // We will ignore values outside the range of measurement, this will happen around 2.7 -0.4 v
    return;
